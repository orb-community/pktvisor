...
static const char USAGE[] =
    R"(pktvisor-dnstap
    Usage:
      pktvisor-dnstap [options] DNSTAP
      pktvisor-dnstap (-h | --help)
      pktvisor-dnstap --version

    Summarize a dnstap binqary file. The result will be written to stdout in JSON format, while console logs will be printed
    to stderr.
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        auto input_stream = std::make_unique<input::dnstap::DnstapInputStream>("dnstap");
        input_stream->config_set("dnstap_file", args["DNSTAP"].asString());
        ...
        auto [input_stream_, stream_mgr_lock] = mgrs.input_manager()->module_get_locked("dnstap");
        stream_mgr_lock.unlock();
        auto dnstap_stream = dynamic_cast<input::dnstap::DnstapInputStream *>(input_stream_);
        ...
        {
            auto handler_module = std::make_unique<handler::net::NetStreamHandler>("net", dnstap_stream, &window_config);
        ...
        {
            auto handler_module = std::make_unique<handler::dns::DnsStreamHandler>("dns", dnstap_stream, &window_config);
            handler_module->config_set("recorded_stream", true);
            handler_module->start();
            mgrs.handler_manager()->module_add(std::move(handler_module));
