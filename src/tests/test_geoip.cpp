...
TEST_CASE("GeoIP", "[geoip]")
{
    SECTION("Geo enablement")
    {
        ...
        CHECK(!visor::geo::GeoIP().enabled());
        CHECK(visor::geo::GeoIP().getGeoLocString("2a02:dac0::").location == "");
    ...
    {
        CHECK(visor::geo::GeoIP().enabled());
        auto loc = visor::geo::GeoIP().getGeoLocString("2a02:dac0::");
        CHECK(loc.location == "EU/Russia");
        CHECK(loc.latitude == "60.000000");
        CHECK(loc.longitude == "100.000000");
        CHECK(visor::geo::GeoIP().getGeoLocString("89.160.20.112").location == "EU/Sweden/E/Linköping");
        CHECK(visor::geo::GeoIP().getGeoLocString("216.160.83.56").location == "NA/United States/WA/Milton");
    }
    ...
    SECTION("basic Geo lookup, socket")
    {
        ...
        CHECK(visor::geo::GeoIP().getGeoLocString(&sa4).location == "EU/Sweden/E/Linköping");
        CHECK(visor::geo::GeoIP().getGeoLocString(&sa4).location == "EU/Sweden/E/Linköping");
        CHECK(visor::geo::GeoIP().getGeoLocString((struct sockaddr *)&sa4).location == "EU/Sweden/E/Linköping");
        struct sockaddr_in6 sa6;
        sa6.sin6_family = AF_INET6;
        inet_pton(AF_INET6, "2a02:dac0::", &sa6.sin6_addr);
        CHECK(visor::geo::GeoIP().getGeoLocString(&sa6).location == "EU/Russia");
        CHECK(visor::geo::GeoIP().getGeoLocString(&sa6).location == "EU/Russia");
        CHECK(visor::geo::GeoIP().getGeoLocString((struct sockaddr *)&sa6).location == "EU/Russia");
    }
...
TEST_CASE("GeoIP without cache", "[geoip]")
{
    ...
    {
        CHECK(visor::geo::GeoIP().enabled());
        CHECK(visor::geo::GeoIP().getGeoLocString("2a02:dac0::").location == "EU/Russia");
    }
