...
void visor::PolicyManager::load(const YAML::Node &tap_yaml, bool strict)
{
    ...
    for (YAML::const_iterator it = tap_yaml.begin(); it != tap_yaml.end(); ++it) {
        ...
        spdlog::get("visor")->info("loading Policy: {}", tap_name);
        if (!it->second.IsMap()) {
        ...
        if (std::find(input_plugins.begin(), input_plugins.end(), input_type) == input_plugins.end()) {
            if (strict) {
                throw ConfigException(fmt::format("Policy '{}' requires input stream type '{}' which is not available", tap_name, input_type));
            } else {
                spdlog::get("visor")->warn("Policy '{}' requires input stream type '{}' which is not available; skipping", tap_name, input_type);
                continue;
            }
        }

        auto tap_module = std::make_unique<Policy>(tap_name, input_type);
