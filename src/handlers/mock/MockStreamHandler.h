...
class MockStreamHandler final : public visor::StreamMetricsHandler<MockMetricsManager>
{
    ...
    MockInputStreamCallback *_mock_stream;
...
public:
    MockStreamHandler(const std::string &name, InputCallback *stream, const Configurable *window_config, StreamHandler *handler = nullptr);
    ~MockStreamHandler();
