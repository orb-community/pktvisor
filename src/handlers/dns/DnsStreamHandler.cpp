...
void DnsStreamHandler::tcp_message_ready_cb(int8_t side, const pcpp::TcpStreamData &tcpData)
{
    ...
    auto got_dns_message = [this, port, dir, l3Type, flowKey, stamp](std::unique_ptr<uint8_t[]> data, size_t size) {
        ...
        if (!_filtering(dnsLayer, dir, l3Type, pcpp::UDP, port, stamp)) {
            _metrics->process_dns_layer(dnsLayer, dir, l3Type, pcpp::TCP, flowKey, port, _static_suffix, stamp);
            _static_suffix = "";
        }
...
void DnsMetricsBucket::process_dns_layer(bool deep, DnsLayer &payload, pcpp::ProtocolType l3, Protocol l4, uint16_t port, std::string_view static_suffix)
{
    std::unique_lock lock(_mutex);
    ...
    if (query) {
        ...
        if (group_enabled(group::DnsMetrics::TopQnames)) {
            ...
            auto aggDomain = aggregateDomain(name, static_suffix);
...
void DnsMetricsManager::process_dns_layer(DnsLayer &payload, PacketDirection dir, pcpp::ProtocolType l3, pcpp::ProtocolType l4, uint32_t flowkey, uint16_t port, std::string_view static_suffix, timespec stamp)
{
    // base event
    new_event(stamp);
    // process in the "live" bucket. this will parse the resources if we are deep sampling
    live_bucket()->process_dns_layer(_deep_sampling_now, payload, l3, static_cast<Protocol>(l4), port, static_suffix);
