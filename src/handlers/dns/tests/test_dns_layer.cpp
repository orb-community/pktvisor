...
TEST_CASE("Parse DNS UDP IPv4 tests", "[pcap][ipv4][udp][dns]")
{
    ...
    auto counters = dns_handler.wire_dns()->metrics()->bucket(0)->counters();
    auto event_data = dns_handler.wire_dns()->metrics()->bucket(0)->event_data_locked();

    CHECK(dns_handler.wire_dns()->metrics()->current_periods() == 1);
    CHECK(dns_handler.wire_dns()->metrics()->start_tstamp().tv_sec == 1567706414);
    CHECK(dns_handler.wire_dns()->metrics()->start_tstamp().tv_nsec == 599964000);

    CHECK(dns_handler.wire_dns()->metrics()->end_tstamp().tv_sec == 1567706420);
    CHECK(dns_handler.wire_dns()->metrics()->end_tstamp().tv_nsec == 602866000);

    CHECK(dns_handler.wire_dns()->metrics()->bucket(0)->period_length() == 6);

    json j;
    dns_handler.wire_dns()->metrics()->bucket(0)->to_json(j);
...
TEST_CASE("Parse DNS random UDP/TCP tests", "[pcap][dns]")
{
    ...
    auto counters = dns_handler.wire_dns()->metrics()->bucket(0)->counters();
    auto event_data = dns_handler.wire_dns()->metrics()->bucket(0)->event_data_locked();
    auto counters_xact = dns_handler.xact_dns()->metrics()->bucket(0)->counters();
    ...
    CHECK(counters_xact.xacts_timed_out.value() == 0);
...
TEST_CASE("DNS Filters: answer_count", "[pcap][dns]")
{
    ...
    c.config_set<uint64_t>("num_periods", 1);
    DnsStreamHandler dns_handler{"dns-test", stream_proxy, &c};
    dns_handler.wire_dns()->config_set<uint64_t>("only_rcode", NoError);
    dns_handler.wire_dns()->config_set<uint64_t>("answer_count", 0);
    dns_handler.start();
    stream.start();
    stream.stop();
    dns_handler.stop();
...
TEST_CASE("DNS groups", "[pcap][dns]")
{
    ...
    SECTION("disable cardinality and counters")
    {
        ...
        CHECK(counters.queries.value() == 0);
        CHECK(counters.replies.value() == 0);
        CHECK(counters_xact.xacts_total.value() == 2921);
        CHECK(counters_xact.xacts_in.value() == 0);
        CHECK(counters_xact.xacts_out.value() == 2921);
        CHECK(counters_xact.xacts_timed_out.value() == 0);

        nlohmann::json j;
        dns_handler.wire_dns()->metrics()->bucket(0)->to_json(j);
        dns_handler.xact_dns()->metrics()->bucket(0)->to_json(j);
    ...
    SECTION("disable TopQname and Dns Transactions")
    {
        ...
        CHECK(counters_xact.xacts_total.value() == 0);
        CHECK(counters_xact.xacts_in.value() == 0);
        CHECK(counters_xact.xacts_out.value() == 0);
        CHECK(counters_xact.xacts_timed_out.value() == 0);
...
TEST_CASE("DNS Filters: only_rcode with predicate", "[pcap][dns][filter]")
{
    ...
    auto event_data_1 = dns_handler_1.wire_dns()->metrics()->bucket(0)->event_data_locked();

    CHECK(event_data_1.num_events->value() == 0);
    CHECK(event_data_1.num_samples->value() == 0);

    auto event_data_2 = dns_handler_2.wire_dns()->metrics()->bucket(0)->event_data_locked();

    CHECK(event_data_2.num_events->value() == 1);
    CHECK(event_data_2.num_samples->value() == 1);

    nlohmann::json j;
    dns_handler_2.wire_dns()->metrics()->bucket(0)->to_json(j);
}
