...
std::unique_ptr<StreamHandler> DhcpHandlerModulePlugin::instantiate(const std::string &name, InputCallback *input_stream_cb, const Configurable *config, const Configurable *filter, StreamHandler *stream_handler)
{
    // TODO using config as both window config and module config
    auto handler_module = std::make_unique<DhcpStreamHandler>(name, input_stream_cb, config, stream_handler);
