...
TEST_CASE("Parse DNS UDP IPv4 tests", "[pcap][ipv4][udp][dns]")
{
    ...
    DnsWireStreamHandler dns_handler{"dns-test", stream_proxy, &c};
...
TEST_CASE("DNS Filters: only_rcode with predicate", "[pcap][dns][filter]")
{
    ...
    auto stream_proxy = stream.add_event_proxy(c);
    c.config_set<uint64_t>("num_periods", 1);
    DnsWireStreamHandler dns_handler_1{"dns-test-1", stream_proxy, &c};
    DnsWireStreamHandler dns_handler_2{"dns-test-2", stream_proxy, &c};
