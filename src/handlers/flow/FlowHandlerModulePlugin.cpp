...
std::unique_ptr<StreamHandler> FlowHandlerModulePlugin::instantiate(const std::string &name, InputEventProxy *proxy, const Configurable *config, const Configurable *filter, HandlerEventProxy *h_proxy)
{
    // TODO using config as both window config and module config
    auto handler_module = std::make_unique<FlowStreamHandler>(name, proxy, config, h_proxy);
