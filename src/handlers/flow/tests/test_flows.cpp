...
TEST_CASE("Parse sflow with enrichment", "[sflow][flow]")
{
    ...
    c.config_set<uint64_t>("num_periods", 1);
    FlowStreamHandler flow_handler{"flow-test", stream_proxy, &c};
    flow_handler.config_set<visor::Configurable::StringList>("device_map", {"route1,192.168.0.11,eth0,37,provide Y", "route2,192.168.0.12,eth0,4,provide X"});
    flow_handler.config_set<std::string>("concatenate_interface", "eth1");
    ...
    nlohmann::json j;
    flow_handler.metrics()->bucket(0)->to_json(j);
    CHECK(j["devices"]["route1|eth1"]["top_in_if_index_bytes"][0]["name"] == "eth0");
    CHECK(j["devices"]["route2|eth1"]["top_in_if_index_bytes"][0]["name"] == "eth0");
    FlowStreamHandler::enrich_data.clear();
}
