...
void FlowMetricsBucket::to_prometheus(std::stringstream &out, Metric::LabelMap add_labels) const
{
    ...
    for (const auto &device : _devices_metrics) {
        ...
        if (group_enabled(group::FlowMetrics::TopByBytes)) {
            ...
            device.second->topByBytes.topSrcPort.to_prometheus(out, device_labels, [](const visor::IpPort &val) { return val.get_name(); });
            device.second->topByBytes.topDstPort.to_prometheus(out, device_labels, [](const visor::IpPort &val) { return val.get_name(); });
        ...
        if (group_enabled(group::FlowMetrics::TopByPackets)) {
            ...
            device.second->topByPackets.topSrcPort.to_prometheus(out, device_labels, [](const visor::IpPort &val) { return val.get_name(); });
            device.second->topByPackets.topDstPort.to_prometheus(out, device_labels, [](const visor::IpPort &val) { return val.get_name(); });
...
void FlowMetricsBucket::to_json(json &j) const
{
    ...
    for (const auto &device : _devices_metrics) {
        ...
        if (group_enabled(group::FlowMetrics::TopByBytes)) {
            ...
            device.second->topByBytes.topSrcPort.to_json(j["devices"][deviceId], [](const visor::IpPort &val) { return val.get_name(); });
            device.second->topByBytes.topDstPort.to_json(j["devices"][deviceId], [](const visor::IpPort &val) { return val.get_name(); });
        ...
        if (group_enabled(group::FlowMetrics::TopByPackets)) {
            ...
            device.second->topByPackets.topSrcPort.to_json(j["devices"][deviceId], [](const visor::IpPort &val) { return val.get_name(); });
            device.second->topByPackets.topDstPort.to_json(j["devices"][deviceId], [](const visor::IpPort &val) { return val.get_name(); });
...
void FlowMetricsBucket::process_flow(bool deep, const FlowPacket &payload)
{
    ...
    for (const auto &flow : payload.flow_data) {
        ...
        auto proto = visor::Protocol::TCP;
        if (flow.l4 == IP_PROTOCOL::UDP) {
            proto = visor::Protocol::UDP;
        }

        if (group_enabled(group::FlowMetrics::TopByBytes)) {
            (flow.src_port > 0) ? device_flow->topByBytes.topSrcPort.update(visor::IpPort{flow.src_port, proto}, flow.payload_size) : void();
            (flow.dst_port > 0) ? device_flow->topByBytes.topDstPort.update(visor::IpPort{flow.dst_port, proto}, flow.payload_size) : void();
        ...
        if (group_enabled(group::FlowMetrics::TopByPackets)) {
            (flow.src_port > 0) ? device_flow->topByPackets.topSrcPort.update(visor::IpPort{flow.src_port, proto}, flow.packets) : void();
            (flow.dst_port > 0) ? device_flow->topByPackets.topDstPort.update(visor::IpPort{flow.dst_port, proto}, flow.packets) : void();
