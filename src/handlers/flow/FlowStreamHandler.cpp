...
void FlowMetricsBucket::specialized_merge(const AbstractMetricsBucket &o, bool aggregate)
{
    // static because caller guarantees only our own bucket type
    const auto &other = static_cast<const FlowMetricsBucket &>(o);

    // rates maintain their own thread safety
    _rate.merge(other._rate, aggregate);
    _throughput.merge(other._throughput, aggregate);
    ...
    _volume.merge(other._volume, aggregate);

    for (const auto &device : other._devices_metrics) {
        const auto &deviceId = device.first;
        const auto &device_data = device.second;

        _devices_metrics[deviceId]->payload_size.merge(device.second->payload_size, aggregate);
